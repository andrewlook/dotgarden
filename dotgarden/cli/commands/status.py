...
def _dotfiles_display(target_str, home_dir):
    """Replace the dotfiles repo path with $DOTFILES/ in display strings."""
    if target_str.startswith('~/dotfiles/'):
        return '$DOTFILES/' + target_str[len('~/dotfiles/') :]
    return target_str


def _print_entry_rows(entries, dotfiles_dir, indent='    ', local_statuses=None):
    ...
    for entry, status, source, target, is_ok in rows:
        ...
        target_display = _dotfiles_display(paths.escape_spaces(target), home_dir)
...
def cmd_status(args):
    ...
    for group_name, entries in sorted_groups:
        ...
        if cond:
            if not _print_entry_rows(entries, dotfiles_dir, indent):
                all_good = False
            for entry in entries:
                _print_variant_chain(entry, all_local_statuses, indent)
        else:
            if not _print_entry_rows(entries, dotfiles_dir, indent, local_statuses):
                all_good = False
